...
class SchedulerGreedyAllocationOptimizedChunked(SchedulerGreedyAllocation):
    ...
    def __init__(
        ...
        # define vars for debugging
        if self.debug_path:
            self.progress_time: List[float] = []
            self.progress_num_waiting: List[float] = []
            self.progress_num_running: List[float] = []
            self.progress_num_finished: List[float] = []
            self.progress_num_preempted: List[float] = []
    ...
    def debug_chart(self) -> None:
        ...
        np.save(os.path.join(self.debug_path, 'debug_time_array.npy'), self.progress_time)
        np.save(os.path.join(self.debug_path, 'debug_num_waiting_array.npy'), self.progress_num_waiting)
        np.save(os.path.join(self.debug_path, 'debug_num_running_array.npy'), self.progress_num_running)
        np.save(os.path.join(self.debug_path, 'debug_num_finished_array.npy'), self.progress_num_finished)
        np.save(os.path.join(self.debug_path, 'debug_num_preempted_array.npy'), self.progress_num_preempted)
        np.save(os.path.join(self.debug_path, 'debug_blocks_in_use_array.npy'), self.progress_blocks_in_use)
