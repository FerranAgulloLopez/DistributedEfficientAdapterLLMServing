...
def retrieve_all_sharegpt_requests(
    dataset_path: str,
...
class AdapterPromptsGenerator:
    def __init__(
        ...
        if dataset_name == "sharegpt":
            self.available_prompts: List[Tuple[str, int, int]] = retrieve_all_sharegpt_requests(
    ...
    def get_adapter_prompt(self):
        return random.choice(self.available_prompts)
...
async def get_request(
    adapter_prompts_generator: AdapterPromptsGenerator,
    adapter_rate: float,
    finish_time: float
) -> AsyncGenerator[Tuple[str, int, int], None]:
    available_distributions = [
        lambda x: np.random.exponential(1.0 / x),  # poisson
        lambda x: np.random.lognormal(0.0, x)  # lognormal
    ]
    current_distribution = available_distributions[0]
    current_rate_multiplier: float = 1
    ...
    interval = current_distribution(adapter_rate * current_rate_multiplier)
    while True:
        prompt, prompt_len, output_len = adapter_prompts_generator.get_adapter_prompt()
        yield prompt, prompt_len, output_len, interval
        time_counter += interval
        await asyncio.sleep(interval)
        if time_counter > 300:
            # change distribution
            current_distribution = random.choice(available_distributions)
            # change adapter rate
            current_rate_multiplier = random.choice([current_rate_multiplier * 1.5, current_rate_multiplier / 1.5])
            current_rate_multiplier = min(current_rate_multiplier, 1.5 * 5)  # clip top
            current_rate_multiplier = max(current_rate_multiplier, 1 / (1.5 * 5))  # clip bottom
            # update time counter
            time_counter = 0
        interval = current_distribution(adapter_rate * current_rate_multiplier)
...
async def send_adapter_requests(
    ...
    async for (prompt, prompt_len, output_len, next_interval) in get_request(adapter_prompts_generator, adapter_rate, finish_time):
...
async def benchmark(
    ...
    if not lora_pre_loading:
        ...
        test_prompt, test_prompt_len, test_output_len = adapter_prompts_generator.get_adapter_prompt()
    ...
    for adapter_index, adapter in enumerate(adapters):
        tasks.append(asyncio.create_task(
            send_adapter_requests(
                ...
                adapter_prompts_generator=adapter_prompts_generator,
...
def main(args: argparse.Namespace):
    ...
    try:
        ...
        adapter_prompts_generator: AdapterPromptsGenerator = AdapterPromptsGenerator(
