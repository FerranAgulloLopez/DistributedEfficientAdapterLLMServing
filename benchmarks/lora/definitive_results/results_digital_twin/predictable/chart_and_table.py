...
ML_MODEL_PATH: Dict[str, str] = {
    'llama-3.1-8b-instruct': '../../results_ml/training/default/llama-3.1-8b-instruct/throughput/rf/',
    'qwen-2.5-7b-instruct': '../../results_ml/training/default/qwen-2.5-7b-instruct/throughput/knn/',
}
    ...
    for index, cpu_loras in enumerate(cpu_loras_values):
        ...
        for rate, x_line, _, z_line in processed_results:
            ...
            for index in range(len(x_line)):
                ...
                if os.path.exists(simulation_full_path) and os.path.exists(os.path.join(simulation_full_path, 'simulation_results.json')):
                    # load output
            ...
            # ml results
            for index in range(len(x_line)):
                experiment_config = z_line[index]
                rates_full: np.ndarray = experiment_config['rates_full']
                sizes_full: np.ndarray = experiment_config['sizes_full']
                input_data = {  # TODO use full rates and sizes
                    'sum_rate': float(np.sum(rates_full)),
                    'std_rate': float(np.std(rates_full)),
                    'max_size': float(np.max(sizes_full)),
                    'mean_size': float(np.mean(sizes_full)),
                    'std_size': float(np.std(sizes_full)),
                    'served_adapters': experiment_config['cpu_loras'],
                    'adapter_slots': experiment_config['m']
                }
                ...
                output_data = predict_digital_twin(
                    ...
                    y_features_to_predict=['total_throughput', 'itl', 'ttft'],
                )
                for metric, value in output_data.items():
                    if rate not in ml_results[cpu_loras][metric]:
                        ml_results[cpu_loras][metric][rate] = []
                    if metric == 'itl' or metric == 'ttft':
                        ml_results[cpu_loras][metric][rate].append(None)
                    else:
                        ml_results[cpu_loras][metric][rate].append(value)
...
def plot_results_small_5(
        ...
        ml_results_list: List[Dict[int, Dict[str, Dict[str, List[float]]]]],
        path: str,
        title: str,
    ...
    slots_value = 8
    cpu_loras_values = [8, 16, 32, 64, 128, 192, 256, 320, 384]
    ...
    rate_label = {
        'rates 0.1 0.05 0.025': 'Low rates',
        'rates 1.6 0.8 0.4': 'High rates',
    }
    ...
    for index_x, metric in enumerate(metrics_labels.keys()):
        ...
        lines = []
        for rate in real_results_list[index_y][cpu_loras_values[0]][metric].keys():
            x_line = cpu_loras_values
            y_line_real = []
            ...
            for cpu_loras in cpu_loras_values:
                slots_index = None
                index_array = 0
                while slots_index is None and index_array < len(real_results_list[index_y][cpu_loras]['m'][rate]):
                    if real_results_list[index_y][cpu_loras]['m'][rate][index_array] == slots_value:
                        slots_index = index_array
                if slots_index is not None:
                    y_line_real.append(real_results_list[index_y][cpu_loras][metric][rate][slots_index])
                    y_line_simulated_simplified.append(simulated_simple_results_list[index_y][cpu_loras][metric][rate][slots_index])
                    y_line_ml.append(ml_results_list[index_y][cpu_loras][metric][rate][slots_index])

            ''' if rate in simulated_simple_results_list[index_y][cpu_loras_values[0]][metric]:
                            y_line_simulated_simplified = []
                            for cpu_loras in cpu_loras_values:
                                slots_index = None
                                index_array = 0
                                while slots_index is None and index_array < len(simulated_simple_results_list[index_y][cpu_loras]['m'][rate]):
                                    if simulated_simple_results_list[index_y][cpu_loras]['m'][rate][index_array] == slots_value:
                                        slots_index = index_array
                                if slots_index is not None:
                                    y_line_simulated_simplified.append(simulated_simple_results_list[index_y][cpu_loras][metric][rate][slots_index])
                        else:
                            y_line_simulated_simplified = None
                        if rate in ml_results_list[index_y][cpu_loras_values[0]][metric]:
                            y_line_ml = []
                            for cpu_loras in cpu_loras_values:
                                slots_index = None
                                index_array = 0
                                while slots_index is None and index_array < len(ml_results_list[index_y][cpu_loras]['m'][rate]):
                                    if ml_results_list[index_y][cpu_loras]['m'][rate][index_array] == slots_value:
                                        slots_index = index_array
                                if slots_index is not None:
                                    y_line_ml.append(ml_results_list[index_y][cpu_loras][metric][rate][slots_index])
                        else:
            ...
            if y_line_simulated_simplified is not None:
                if len(y_line_simulated_simplified) == len(x_line):
                    ...
                    print('Wow')
            if y_line_ml is not None:
                if len(y_line_ml) == len(x_line):
                    axs[index_x].plot(
                        x_line,
                        y_line_ml,
                        marker='',
    ...
    legend_elements.append(Line2D([], [], color='black', linestyle='dotted', label='ML'))
    ...
    plt.savefig(os.path.join(path, f'results_small_{title.replace(".", "")}_5.pdf'), format='pdf', bbox_inches='tight', dpi=400)
...
def create_table(
    ...
    for metric in list(metrics.keys()) + ['cpu_usage', 'memory_usage']:
        ...
        for cpu_loras in cpu_loras_values:
            ...
            for index_x, label_results in enumerate(labels_list):
                for rate in simulated_results_list[index_x][cpu_loras][metric].keys():
                    if metric not in {'cpu_usage', 'memory_usage'}:
    ...
    for metric in real_results.keys():
        ...
        if len(real_results[metric]) > 0:
            for distance_label, distance_metric in [('SMAPE', smape)]:
                default: float = distance_metric(real_results[metric], simulated_results[metric])
    ...
    print(
        '\n\nCPU usage',
        '. Default+Simple:', print_metric_values(mean_margin_of_error(simulated_results['cpu_usage'] + simulated_simple_results['cpu_usage'])),
        # '. Default preemption:', print_metric_value(simulated_duration_preemption),
        # '. Simple preemption:', print_metric_value(simulated_simple_duration_preemption)
    )
    print(
        '\n\nMemory usage',
        '. Default+Simple:', print_metric_values(mean_margin_of_error(simulated_results['memory_usage'] + simulated_simple_results['memory_usage'])),
        # '. Default preemption:', print_metric_value(simulated_duration_preemption),
        # '. Simple preemption:', print_metric_value(simulated_simple_duration_preemption)
    )
