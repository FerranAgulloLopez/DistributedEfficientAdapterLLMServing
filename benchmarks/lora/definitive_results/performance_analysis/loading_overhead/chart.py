...
def plot_loading_overhead(
        loading_results: Dict[str, Dict[str, Dict[str, float]]],
        path: str,
        title: str,
) -> None:
    first_load_data: List[float] = []
    next_load_data: List[float] = []
    for rank in ['rank_8', 'rank_16', 'rank_32']:
        first_load_data.append(
            (
                loading_results[rank]['first_load']['disk->CPUMem'] +
                loading_results[rank]['first_load']['CPUMem->GPUMem']
            ) * 1000
        )
        next_load_data.append(
            (
                    loading_results[rank]['consecutive_loads']['disk->CPUMem'] +
                    loading_results[rank]['consecutive_loads']['CPUMem->GPUMem']
            ) * 1000
        )

    first_load_data = np.asarray(first_load_data)
    next_load_data = np.asarray(next_load_data)
    ...
    for label_results, y_line in {
        'first load': first_load_data,
        'average next loads': next_load_data,
    }.items():
        offset = width * multiplier
        rects = ax.bar(x_line + offset, y_line, width, label=label_results)
        # ax.bar_label(rects, padding=3)
        multiplier += 1
    ax.set_ylabel('loading time (ms)', fontsize=10)
    # ax.set_xlabel('')
    # ax.set_title('')
    ax.set_xticks(x_line + width, x_line_labels)
    ax.legend(loc='upper left', ncols=3, fontsize=10)

    plt.savefig(os.path.join(path, f'loading_overhead_{title}'), bbox_inches='tight')
...
def plot_latency_overhead(
    ...
    new_labels = {
        'SmallRequest': '23/27 lengths',
        'MediumRequest': '250/230 lengths',
        'LargeRequest': '423/357 lengths',
    }
    ...
    plt.savefig(os.path.join(path, f'loading_overhead_latency_{title}.pdf'), format='pdf', bbox_inches='tight', dpi=400)


def plot_throughput_overhead(
        throughput_results: List[Dict[str, float]],
        loading_results: Dict[str, Dict[str, Dict[str, float]]],
        path: str,
        title: str,
) -> None:
    for exp in throughput_results:
        rank: int = exp['rank']
        duration: float = exp['duration'] * 1000
        output_tokens: int = exp['output_tokens']
        loading_time: float = \
            (
                    loading_results[f'rank_{rank}']['consecutive_loads']['disk->CPUMem'] + \
                    loading_results[f'rank_{rank}']['consecutive_loads']['CPUMem->GPUMem']
            ) * 1000
        throughput: float = output_tokens / duration
        throughput_with_loading: float = output_tokens / (duration + loading_time)
        exp['throughput_overhead'] = (1 - throughput_with_loading / throughput) * 100

    rank_lines = __prepare_lines(
        throughput_results,
        'prompts',
        'throughput_overhead',
        'rank'
    ...
    for rank, x_line, y_line in rank_lines:
        line = axs.plot(
            x_line,
            y_line,
            marker='o',
    ...
    axs.set_xlabel('requests (#)', fontsize=10)
    axs.set_ylabel('throughput slowdown by loading (%)', fontsize=10)
    axs.legend(loc='upper right', fontsize=10)

    plt.savefig(os.path.join(path, f'loading_overhead_throughput_{title}'), bbox_inches='tight')
...
def main():
    ...
    results_p25: List[Dict[str, float]] = merge_results(extract_results_throughput(f'throughput/with_pre_loading/{model}/p25_dataset'), ['m', 'rank', 'prompts'], {'output_tokens', 'duration'})
    results_mean: List[Dict[str, float]] = merge_results(extract_results_throughput(f'throughput/with_pre_loading/{model}/mean_dataset'), ['m', 'rank', 'prompts'], {'output_tokens', 'duration'})
    results_p75: List[Dict[str, float]] = merge_results(extract_results_throughput(f'throughput/with_pre_loading/{model}/p75_dataset'), ['m', 'rank', 'prompts'], {'output_tokens', 'duration'})
    throughput_results: Dict[str, List[Dict[str, float]]] = {
        'SmallRequest': results_p25,
        'MediumRequest': results_mean,
        'LargeRequest': results_p75
    }
    ...
    plot_throughput_overhead(
        throughput_results['SmallRequest'],
        loading_results,
        '.',
        ''
    )
