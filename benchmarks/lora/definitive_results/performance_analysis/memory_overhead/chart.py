...
def plot_memory_overhead_new(
    ...
    metrics = ['mean_batch_size', 'total_throughput', 'itl']
    model_aux_results = []
    for label_model, model_results in model_results.items():
        results_aux_list = []
        for label_results, results in model_results.items():
            if label_results != 'MediumRequest':
                continue
            for metric in metrics:
                results_throughput = __prepare_lines(
                    results,
                    'mean_batch_size' if metric == 'itl' else 'm',
                    metric,
                    'rank'
                )
                results_aux_list.append((metric, results_throughput))
        model_aux_results.append((label_model, results_aux_list))

    nrows = 1
    ncols = len(model_aux_results[0][1])
    ...
    for label_model, results_throughput_list in model_aux_results:
        for index_x, (label_results, results_throughput) in enumerate(results_throughput_list):
            ...
            '''if label_results == 'mean_batch_size':
                axs[index_x].set_ylabel('batch size (#)')
            else:
            ...
            if label_results == 'mean_batch_size':
                axs[index_x].set_title('Batch size (#)')
            elif label_results == 'total_throughput':
                axs[index_x].set_title('Throughput (toks/s)')
            elif label_results == 'itl':
                axs[index_x].set_title('ITL (ms)')

            if label_results == 'mean_batch_size' or label_results == 'total_throughput':
                axs[index_x].set_xlabel('adapters in GPU (#)')
            else:
                axs[index_x].set_xlabel('batch size (#)')
            # axs[index_x].set_title(label_results)

    legend_elements = []
    for model, linestyle in linestyles.items():
        legend_elements.append(Line2D([], [], color='gray', marker=symbols[model], linestyle=linestyle, label=model))
    ...
    plt.savefig(os.path.join(path, f'memory_overhead_new_{title}.pdf'), format='pdf', bbox_inches='tight', dpi=400)
