...
def plot_results_together(
        results_sizes_left: List[Dict[str, Any]],
        results_sizes_right: List[Dict[str, Any]],
        path: str,
        x_metric: str,
        x_label: str,
        y_metric: str,
    ...
    markers_width = {
        ...
        'baseline-4-with-proposal': 20 * multiplier_2,
    }
    good_labels = {
        'baseline-3': 'Random',
    ...
    colors = {'proposal-starvation': '#1f77b4', 'lora-serve': '#ff7f0e', 'lora-serve-half': '#2ca02c', 'dlora-proactive-mechanism': '#d62728', 'baseline-4-with-proposal': '#6fc0f7'}
    nrows = 1
    ncols = 2
    ...
    for index_x, (x_axis_label, results) in enumerate([('Llama', results_sizes_left), ('Qwen', results_sizes_right)]):
        ...
        if index_x == 0:
            axs[index_x].set_ylim(13, 29)
            axs[index_x].set_yticks([15, 20, 25])
            axs[index_x].set_ylabel(y_label)
        else:
            axs[index_x].set_ylim(0, 200)
            axs[index_x].set_yticks([0, 50, 100, 150])
        ...
        axs[index_x].set_title(x_axis_label)
    ...
    plt.savefig(os.path.join(path, f'latency.pdf'), format='pdf', bbox_inches='tight', dpi=400)


def main():
    model = 'llama-3.1-8b-instruct'
    sizes = 'sizes_8-16'
    results_sizes_left: List[Dict[str, Any]] = extract_results(f'../multi_gpu/results_{sizes}/{sizes}/{model}')
    list_of_results: Dict[int, List[Dict[str, Any]]] = {}
    for result in results_sizes_left:
    ...
    results_sizes_left = list_of_results[10]

    model = 'qwen-2.5-7b-instruct'  # change to single-GPU exps
    sizes = 'sizes_8-16-32'
    results_sizes_right: List[Dict[str, Any]] = extract_results(f'../single_gpu/results/{sizes}/{model}')
    list_of_results: Dict[int, List[Dict[str, Any]]] = {}
    for result in results_sizes_right:
        '''if result['concurrent_adapters'] in {64, 96, 128, 384, 640, 896, 1152}:
            continue'''
        trace_arrival_multiplier = result['trace_arrival_multiplier']
        if trace_arrival_multiplier not in list_of_results:
            list_of_results[trace_arrival_multiplier] = []
        list_of_results[trace_arrival_multiplier].append(result)
    results_sizes_right = list_of_results[100]

    plot_results_together(
        results_sizes_left=results_sizes_left,
        results_sizes_right=results_sizes_right,
        path='.',
        y_metric='itl',
        y_label='ITL (ms)',
        x_metric='concurrent_adapters',
        x_label='adapters to serve (#)',
        algorithms_to_use=['lora-serve', 'dlora-proactive-mechanism', 'baseline-4-with-proposal'],
    )
