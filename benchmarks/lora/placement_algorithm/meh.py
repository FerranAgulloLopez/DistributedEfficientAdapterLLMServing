...
from factory import get_subclass, get_subclass_dynamic_routing, get_subclass_dynamic_routing_with_probabilities
import numpy as np
...
ML_MODEL_PATH_THROUGHPUT_FAST: Dict[str, str] = {
    'llama-3.1-8b-instruct': '/home/ferran/Documents/repositories/vLLMAdapterServingScaling/benchmarks/lora/definitive_results/finding_maximum/digital_twin_simplification/training/llama-3.1-8b-instruct/explainability/reg/rf_simpler/',
    'qwen-2.5-7b-instruct': '/home/ferran/Documents/repositories/vLLMAdapterServingScaling/benchmarks/lora/definitive_results/finding_maximum/digital_twin_simplification/training/qwen-2.5-7b-instruct/explainability/reg/rf_simpler_2/',
}
ML_MODEL_PATH_STARVATION_FAST: Dict[str, str] = {
    'llama-3.1-8b-instruct': '/home/ferran/Documents/repositories/vLLMAdapterServingScaling/benchmarks/lora/definitive_results/finding_maximum/digital_twin_simplification/training/llama-3.1-8b-instruct/explainability/class/rf_simpler/',
    'qwen-2.5-7b-instruct': '/home/ferran/Documents/repositories/vLLMAdapterServingScaling/benchmarks/lora/definitive_results/finding_maximum/digital_twin_simplification/training/qwen-2.5-7b-instruct/explainability/class/rf_f2_rules_200_depth_14/',
}
...
ADAPTERS = 1024  # 512, 1024, 1280, 1535
SERVERS = 4
...
# METHOD = 'baseline-2'
# METHOD = 'proposal-starvation-2'
METHOD = 'dlora-proactive-mechanism'
FACTORY_METHOD = get_subclass_dynamic_routing  # change

adapters = [f'a{index}' for index in range(0, ADAPTERS)]

servers = [f's{index}' for index in range(0, SERVERS)]
ranks_to_use = [8, 16, 32]
rates_to_use = [0.075, 0.0375, 0.01875, 0.009375, 0.0046875]  # [2.4, 1.2, 0.01875, 0.009375, 0.0046875]
...
if 'fast' in METHOD:
    ...
    servers_adapter_slots, adapters_servers = placement_algorithm.define_placement(
            ...
            ml_model_path_throughput=ML_MODEL_PATH_THROUGHPUT if not 'fast' in METHOD else ML_MODEL_PATH_THROUGHPUT_FAST,
            ml_model_path_starvation=ML_MODEL_PATH_STARVATION if not 'fast' in METHOD else ML_MODEL_PATH_STARVATION_FAST,
        )

# TEST PROPOSAL PLACEMENT
init_time: float = time.perf_counter()
placement_algorithm: PlacementAlgorithmInterface = FACTORY_METHOD(METHOD)()
...
elif METHOD == 'lora-serve':
    servers_adapter_slots, adapters_servers, adapters_servers_probabilities = placement_algorithm.define_placement(
        model=MODEL,
        servers=servers,
        adapters=adapters,
        adapters_ranks=adapters_ranks,
