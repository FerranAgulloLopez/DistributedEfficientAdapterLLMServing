...
async def get_request(
    ...
    adapter_arrivals: List[float],
    start_time: float,
    finish_time: float,
) -> AsyncGenerator[Tuple[str, int, int], None]:
    if not adapter_arrivals:
        yield None, None, None
    index_arrival: int = 0
    while index_arrival < len(adapter_arrivals):
        prompt, prompt_len, output_len = adapter_requests[index_arrival]
        arrival = adapter_arrivals[index_arrival]
        if (start_time + arrival) >= finish_time:
            yield None, None, None
        else:
            await asyncio.sleep(start_time + arrival - time.perf_counter())
...
async def send_adapter_requests(
    ...
    async for (prompt, prompt_len, output_len) in get_request(adapter_requests, adapter_arrivals, start_time, finish_time):
        if prompt is None:
            break
        inputs.append((prompt, prompt_len, output_len))
        api_url: str = random.choices(api_urls, weights=api_urls_weights, k=1)[0]
...
async def benchmark(
    ...
    total_time: int,
    adapters: List[str],
    adapters_arrivals: List[List[float]],
    adapters_prompts: List[List[Tuple[str, int, int]]],
    adapters_api_urls: Dict[str, List[str]],
    adapters_api_urls_weights: Dict[str, List[float]],
):
    ...
    test_prompt, test_prompt_len, test_output_len = None, None, None
    index_prompt = 0
    while test_prompt is None and index_prompt < len(adapters_prompts):
        if adapters_prompts[index_prompt]:
            test_prompt, test_prompt_len, test_output_len = adapters_prompts[index_prompt][0]
        else:
            index_prompt += 1
    assert test_prompt is not None

    aux_list = []
    for aux_values in adapters_api_urls.values():
        aux_list += aux_values
    unique_api_urls = set(aux_list)
    ...
    for adapter_index, adapter in enumerate(adapters):
        tasks.append(asyncio.create_task(
            send_adapter_requests(
                ...
                adapter_arrivals=adapters_arrivals[adapter_index],
                api_urls=adapters_api_urls[adapters[adapter_index]],
                api_urls_weights=adapters_api_urls_weights[adapters[adapter_index]],
...
def main(args: argparse.Namespace, placement_type: str):
    ...
    try:
        ...
        total_prompts: List[Tuple[str, int, int]] = create_prompts(
            total_prompts_size=total_prompts_size,
        ...
        for index_adapter, adapter_server_list in enumerate(adapters_servers):
            adapters_api_urls[adapters[index_adapter]] = []
            for adapter_server in adapter_server_list:
                adapters_api_urls[adapters[index_adapter]].append(get_api_url(args.host, args.port, args.endpoint, server_indexes[adapter_server]))
            adapters_api_urls_weights[adapters[index_adapter]] = adapters_servers_probabilities[index_adapter]
        ...
        benchmark_result, all_arrivals = asyncio.run(
            benchmark(
                ...
                adapters=adapters,
                adapters_arrivals=adapters_arrivals,
                adapters_prompts=adapters_prompts,
                adapters_api_urls=adapters_api_urls,
                adapters_api_urls_weights=adapters_api_urls_weights,
            ))
        ...
        if args.save_result:
            ...
            with open(os.path.join(args.result_dir, "adapters.csv"), mode='w', newline='') as file:
                ...
                writer.writerow(["adapter_id", "adapter_path", "adapter_rank", "adapter_rate", "server", "server_api_urls"])
                for index in range(len(adapters)):
                    api_urls = '+++'.join(adapters_api_urls[adapters[index]])
                    writer.writerow([adapters[index], adapters_paths[index], adapters_ranks[index], adapters_rates[index], adapters_servers[index], api_urls])
