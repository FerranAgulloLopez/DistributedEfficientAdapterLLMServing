...
import sys
sys.path.append("benchmarks/lora")
...
CACHED_MODELS_REG_FAST = None
CACHED_MODELS_CLASS_FAST = None
def predict_digital_twin_fast(
        regression: bool,
        model_path: str,
        x_features: Dict[str, Any],
        y_features_to_predict: Optional[List[str]] = None,
) -> Dict[str, Any]:
    global CACHED_MODELS_REG_FAST, CACHED_MODELS_CLASS_FAST
    ...
    # load models
    if regression:
        if CACHED_MODELS_REG_FAST is None:
            models = []
            for y_feature_label in y_features:
                models.append(joblib.load(os.path.join(model_path, f'best_model_{y_feature_label}_fast.pkl')))
            CACHED_MODELS_REG_FAST = models
        else:
            models = CACHED_MODELS_REG_FAST
    else:
        if CACHED_MODELS_CLASS_FAST is None:
            models = []
            for y_feature_label in y_features:
                models.append(joblib.load(os.path.join(model_path, f'best_model_{y_feature_label}_fast.pkl')))
            CACHED_MODELS_CLASS_FAST = models
        else:
            models = CACHED_MODELS_CLASS_FAST
