...
def simulation_pipeline(
    ...
    if include_network_collapse:
        waiting_file_path = os.path.join(experiment_path, 'num_waiting.npy')
        if not os.path.exists(waiting_file_path):
            waiting_file_path = os.path.join(experiment_path, 'num_waiting_.npy')
        max_progress_num_waiting = int(np.max(np.load(waiting_file_path)))
...
if __name__ == '__main__':
    ...
    if args.debug_path is not None:
        args.debug_path = args.output_path
